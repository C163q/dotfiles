...
function M.line_signs(win, buf, lnum)
  ...
  -- %{foldlevel(v:lnum) > 0 ? (foldlevel(v:lnum) > foldlevel(v:lnum - 1) ?
  -- (foldclosed(v:lnum) == -1 ? "󰍝" : "󰍟") : "│") : " "
  -- Get fold signs
  vim.api.nvim_win_call(win, function()
    if vim.fn.foldlevel(lnum) > 0 and vim.fn.foldlevel(lnum) > vim.fn.foldlevel(lnum - 1) then
    ...
    elseif vim.fn.foldlevel(lnum) > 0 then
      signs[#signs+1] = { text = vim.opt.fillchars:get().foldsep or '│', type = "fold" }
    end
--[[
    if vim.fn.foldclosed(lnum) >= 0 then
      signs[#signs + 1] = { text = vim.opt.fillchars:get().foldclose or "", texthl = "Folded", type = "fold" }
